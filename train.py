...
from utils import L2_loss, custom_loss, to_pickle, from_pickle
...
def get_args():
    ...
    parser.add_argument('--input_dim', default=2*dim*nbodies, type=int, help='dimensionality of input tensor')
    ...
    parser.add_argument('--name', default='wh', type=str, help='only one option right now')
...
def train(args):
  ...
  x = torch.tensor(data['coords'].reshape(data['coords'].shape[0]*data['coords'].shape[1],6), requires_grad=True, dtype=torch.float32).cuda()
  # test_x = torch.tensor( data['test_coords'], requires_grad=True, dtype=torch.float32).cuda()
  test_x = torch.tensor(data['test_coords'].reshape(data['test_coords'].shape[0]*data['test_coords'].shape[1],6), requires_grad=True, dtype=torch.float32).cuda()
...
#     test_dxdt_hat += args.input_noise * torch.randn(*test_x[test_ixs].shape).cuda() # add noise, maybe
