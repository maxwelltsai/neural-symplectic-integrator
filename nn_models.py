...
class MLP(torch.nn.Module):
  ...
  def __init__(self, input_dim, hidden_dim, output_dim, nonlinearity='tanh'):
    super(MLP, self).__init__()
    self.linear1 = torch.nn.Linear(input_dim, hidden_dim*10)
    self.linear2 = torch.nn.Linear(hidden_dim*10, hidden_dim*10)
    self.linear3 = torch.nn.Linear(hidden_dim*10, output_dim, bias=None)

    for l in [self.linear1, self.linear2, self.linear3]:
      torch.nn.init.orthogonal_(l.weight) # use a principled initialization
      
    if not os.path.isfile('mask_file'):
      mask = torch.tensor(sparse.random(hidden_dim*10, hidden_dim*10, density=0.2).toarray())
